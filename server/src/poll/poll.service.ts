...
export class PollService {
    ...
    constructor() {
        ...
        this.contract = new this.web3.eth.Contract(abi, process.env.COTRANCT_ADDRESS);
        this.contract.options.address = process.env.CONTRACT_ADDRESS;
    }

    async getPollNumber(): Promise<number> {
        let cnt: number = parseInt(await this.contract.methods.getPollNumber()
    ...
    async vote(requestDto: voteRequestDto): Promise<string> {
        ...
        let { blockHash } = await this.contract.methods.vote(poll, this.web3.utils.asciiToHex(candidate))
    ...
    async setStatus(requestDto: setStatusRequestDto): Promise<string> {
        ...
        let { blockHash } = await this.contract.methods.setPollStatus(poll, open)
            .send({ from: process.env.DEPLOYER_ADDRESS, gas: 6721975 });
        return blockHash;
    }

    async getResult(poll: number): Promise<[string[], number[]]> {
        let { candidates, result }: { candidates: string[], result: number[] } = await this.contract.methods.getResult(poll)
            .call({ from: process.env.DEPLOYER_ADDRESS });
        return [candidates.map(x => this.web3.utils.hexToAscii(x).replace(/\0/g, '')), result];
    }
}